...
class Reward_Competition(RTC):
    ...
    def winning(self):
        ...
        # Create a copy of the DataFrame to avoid modifying self.trials_df
        df_copy = self.trials_df.copy()
    ...
    def compute_lick_da_metrics(self, df=None, mode='standard'):
        ...
        def compute_da_metrics_for_lick(trial_obj, first_PE_after_tones, closest_port_entry_offsets, 
            ...
            iterating over multiple first_PE_after_tone and closest_port_entry_offset values."""
            
            timestamps = np.array(trial_obj.timestamps)  
            zscores = np.array(trial_obj.zscore)  

            computed_metrics = []
            for first_PE_after_tone, closest_port_entry_offset in zip(first_PE_after_tones, closest_port_entry_offsets):
                # Ensure timestamps array is not empty and start_time is before end_time
                if len(timestamps) == 0 or first_PE_after_tone >= closest_port_entry_offset: 
                    computed_metrics.append({"AUC": np.nan, "Max Peak": np.nan, "Time of Max Peak": np.nan, "Mean Z-score": np.nan, "Adjusted End": np.nan})
                    continue

                # Extract initial window
                mask = (timestamps >= first_PE_after_tone) & (timestamps <= closest_port_entry_offset)
                ...
                if use_adaptive and max_peak >= 0:
                    ...
                    if fall_idx < len(window_z):
                        ...
                        extended_mask = (timestamps >= first_PE_after_tone)
                ...
                final_mask = (timestamps >= first_PE_after_tone) & (timestamps <= closest_port_entry_offset)
        ...
        if mode == 'standard':
            ...
            df["lick_computed_metrics"] = df.apply(
                lambda row: compute_da_metrics_for_lick(row["trial"], row["first_PE_after_sound_cue"], 
    ...
    def reward_comp_vs_alone(self,
                            ...
                            pad_inches=0.1):
        """
        Compare Reward-Training (alone) vs Reward-Competition (competition), and
        compute p-value & Cohen's d.
        """
        # 1) build the actual column we want
        col = f"{behavior} {metric_name}"
        if col not in df_alone.columns or col not in df_comp.columns:
            raise KeyError(f"Column {col} must exist in both dataframes")

        # 2) helper to pick NAc vs mPFC
        def _by_region(df, region):
            df_n = df[df['subject_name'].str.startswith('n')].copy()
            df_p = df[df['subject_name'].str.startswith('p')].copy()
            return df_n if region=='NAc' else df_p

        # 3) collapse each session's list → a single mean
        def _collapse(df):
            sub = df[['subject_name', col]].copy()
            sub[col] = sub[col].apply(lambda x:
                                    np.nanmean(x) if isinstance(x, (list, np.ndarray))
                                    else np.nan)
            return sub.dropna(subset=[col])

        alone = _collapse(_by_region(df_alone, brain_region))
        comp  = _collapse(_by_region(df_comp,  brain_region))

        # 4) get the raw arrays we will test
        arr1 = alone[col].values
        arr2 = comp[col].values

        # 5) compute p-value
        tstat, pvalue = ttest_ind(arr1, arr2,
                                nan_policy='omit',
                                equal_var=False)
        ...
        pooled_sd = np.sqrt(((n1-1)*s1**2 + (n2-1)*s2**2) / (n1 + n2 - 2))
        cohen_d = (np.nanmean(arr1) - np.nanmean(arr2)) / pooled_sd

        # 7) compute plotting values
        mean_alone = np.nanmean(arr1)
        sem_alone  = np.nanstd(arr1, ddof=1) / np.sqrt(n1)
        mean_comp  = np.nanmean(arr2)
        sem_comp   = np.nanstd(arr2, ddof=1) / np.sqrt(n2)

        # 8) delegate to your existing plotting helper
        self.ploting_side_by_side(
            alone[[col]], comp[[col]],
            mean_alone, sem_alone,
            mean_comp, sem_comp,
            nac_color if brain_region=='NAc' else mpfc_color,
            figsize, metric_name,
            ylim, yticks_increment,
            f"{brain_region} {behavior} {metric_name}: Alone vs Competition",
            directory_path, pad_inches,
            'Alone', 'Competition'
        )

        # 9) return stats for logging
        return {
            't_stat':  tstat,
            'p_value': pvalue,
            'cohen_d': cohen_d,
    ...
    def plot_event_index_grid(self,
        ...
        for _, row in df_reg.iterrows():
            ...
            if isinstance(tz, (list, np.ndarray)) and len(tz) > idx:
                ...
                onset       = row['filtered_sound_cues'][idx] \
                              if event_type=='Tone' else row['first_PE_after_sound_cue'][idx]
                peak_abs    = row[f'{event_type} Time of Max Peak'][idx]
                first_PE  = row['first_PE_after_sound_cue'][idx]

                # convert to relative
                peak_rel    = peak_abs - onset
                lick_rel    = first_PE - onset
        ...
        for i, (trace, lbl, onset, peak_info) in enumerate(zip(traces, labels, onset_times, peak_rows)):
            ...
            lick_t = _closest(peak_info['first_PE_s'])
    ...
    def plot_tone_and_lick_peaks_with_first_PE(self,
                                                df: pd.DataFrame,
                                                event_index: int,
                                                brain_region: str,
                                                use_ei: bool = True,
        ...
        for _, row in df_reg.iterrows():
            ...
            fl_abs = row['first_PE_after_sound_cue'][idx]
